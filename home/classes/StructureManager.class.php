...
class StructureManager
{
	...
	function getBody($page) {
		...
		if(count($contents)==1) {
			$path_page = realpath(TR_INCLUDE_PATH		. '../dnd_themod').'/page_template/';;
			...
			if(is_file($file)) {
				...
				if($find) 
						 //$resp[] = str_replace('src="dnd_image', 'src="/dnd_themod/system/page_template_image.png"');
						return str_replace('src="dnd_image', 'src="'.TR_BASE_HREF.'dnd_themod/system/page_template_image.png"', $text);
					 else 	
						return $text;
	...
	function getSrcImgPage($page) {
		...
		foreach ($pages_template as $page_template) {
			$img = TR_BASE_HREF . 'dnd_themod/page_template/' . $page_template . '/screenshot.png';
			$previews[] = $img;
		}
